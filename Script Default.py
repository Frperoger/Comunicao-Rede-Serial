...
#Direto da rede
#Handshakes padrão
HANDSHAKE_NORMAL = {b"\x14"} #DC4(0x14) PC->CNC
HANDSHAKE_DNC = {b"\x12"}#DC2(0x12) DNC
...
def enviar_dnc(conn, caminho, maquina):
    ...
    with open(caminho, "rb") as f:
        for linha in f:
            # Aguarda liberação da CNC (XON) antes de continuar enviando
            while pausado:
                ...
                ctrl = conn.recv(1)
                if not ctrl:
                    raise ConnectionError("Conexão encerrada pela CNC durante pausa XOFF")
                if ctrl == XON:
                    pausado = False
            ...
            if pronto:
                ctrl = conn.recv(1)
                if not ctrl:
                    raise ConnectionError("Conexão encerrada pela CNC durante envio DNC")
                if ctrl == XOFF:
                    log("XOFF recebido - pausa", maquina)
                    pausado = True
                elif ctrl == XON and pausado:
                    pausado = False
                    log("XON recebido - retomando", maquina)
...
def maquina_conectada(nome, ip):
    with lock:
        ...
        maquinas[nome]["atividade"] = "Comunicando"
        maquinas[nome]["ip"] = ip
        maquinas[nome]["last_seen"] = time.time()

def maquina_Online(nome,ip=None):
    with lock:
        if maquinas[nome]["status"] in ("Offline","Handshake","Handshake_DNC","Online"):
